...
# Flatpak export rejects icons above 512x512. The committed
# icons/icon-256.png (pre-scaled, preferred) is used when present;
# otherwise downscale from the 1024px source, with tool fallbacks.
ICON_SRC="$REPO_ROOT/desktop/src-tauri/icons/icon.png"
ICON_256="$REPO_ROOT/desktop/src-tauri/icons/icon-256.png"
if [ -f "$ICON_256" ]; then
    cp "$ICON_256" "$HERE/app-prebuilt/icon.png"
...
PY
    ...
    echo "WARNING: no icon-256.png and no ImageMagick/Pillow found —" \
         "icon stays 1024px; flatpak export will reject it" >&2
fi
